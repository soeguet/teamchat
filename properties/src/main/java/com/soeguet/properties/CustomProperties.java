...
public class CustomProperties extends Properties implements CustomPropertiesInterface {
  ...
  private void createFolderIfNotPresent(final File appDir) {
    ...
    if (!appDir.exists()) {
      ...
      if (!mkdir) {
        ...
        logger.log(
            java.util.logging.Level.SEVERE, "ERROR! Could not createQuoteTopTextPane app dir!");
  ...
  public void save() {
    ...
    //            } catch (IOException e) {
    //
    //                logger.log(java.util.logging.Level.SEVERE, "ERROR: Could not save user " +
    // key, e);
    //                throw new RuntimeException();
    //            }
    //
  ...
  public CustomUserPropertiesDTO loadThisClientProperties() {
    ...
    try {
      ...
      logger.log(
          java.util.logging.Level.SEVERE, "CustomProperties > loadThisClientProperties()", e);
    }
            ...
            key -> {
              try {
                ...
                CustomUserPropertiesDTO userProperties =
                    mapper.readValue(getProperty(key), CustomUserPropertiesDTO.class);
    ...
    this.userPropertiesHashSet.forEach(
        user -> {
          ...
          try {
            ...
            logger.log(
                java.util.logging.Level.SEVERE, "ERROR: Could not save user " + user.username(), e);
            throw new RuntimeException();
          }
  ...
  public CustomUserPropertiesDTO getCustomUserProperties(final String username) {
    ...
    final Optional<CustomUserPropertiesDTO> userProperty =
        userPropertiesHashSet.stream()
            .filter(customUserProperties -> customUserProperties.username().equals(username))
            .findFirst();
