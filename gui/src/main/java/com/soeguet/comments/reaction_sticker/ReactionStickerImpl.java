...
public class ReactionStickerImpl {
  ...
  public ReactionStickerImpl(
      final JLayeredPane layeredPane, final List<UserInteraction> userInteractions) {
  ...
  public synchronized void pasteStickerToContainer() {
    ...
    userInteractions.forEach(
        userInteraction -> {
          final ImageIcon imageIcon = emojiRegister.getEmojiList().get(userInteraction.emoji());
  ...
  /**
   * Revalidates the sticker count based on the maximum number of slots for emotes and the number of
   * available slots in the sticker panel container. This is determined by the width of the
   * container divided by the width of the emote slot. This method hides or shows the components in
   * the sticker panel based on the maximum number of slots for emotes.
   */
