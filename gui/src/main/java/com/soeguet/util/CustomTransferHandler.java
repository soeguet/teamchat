...
public class CustomTransferHandler extends TransferHandler {
  ...
  public CustomTransferHandler() {
    ...
    this.originalTransferHandler =
        ChatMainFrameImpl.getMainFrameInstance().getTextEditorPane().getTransferHandler();
  }
  ...
  /**
   * Creates a link dialog interface with the given link. It checks if there is metadata associated
   * with the link and adds the corresponding metadata panel to the content panel of the dialog if
   * it exists.
   *
